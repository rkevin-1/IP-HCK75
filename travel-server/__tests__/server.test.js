...
describe('Travel Server API', () => {
  ...
  describe('Destination Routes', () => {
    ...
    it('should create a new destination', async () => {
      const res = await request(app)
        .post('/destinations')
        .set('Authorization', `Bearer ${authToken}`)
        .send({ name: 'Test Destination', description: 'A beautiful place', location: 'Test Location', price: 100 });
      expect(res.statusCode).toBe(201);
      expect(res.body).toHaveProperty('id');
  ...
  describe('Review Routes', () => {
    ...
    beforeAll(async () => {
      const destination = await Destination.create({ name: 'Review Test Destination', description: 'For testing reviews', location: 'Test Location', price: 100 });
  ...
  describe('Gemini Routes', () => {
    it('should generate Gemini content', async () => {
        ...
        expect(typeof res.body.candidates[0].content.parts[0].text).toBe('string');
    });
});
